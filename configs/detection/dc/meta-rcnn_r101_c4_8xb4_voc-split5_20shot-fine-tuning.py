...
data_root = 'data/DC/dior_structure'
data = dict(
    samples_per_gpu=2,
    workers_per_gpu=8,
    train=dict(
        ...
        save_dataset=True,
        dataset=dict(
            type='FewShotdcDefaultDataset',
            ann_cfg=[dict(type='ann_file', split='SPLIT5', setting='2SHOT')],
            img_prefix='data/DC/dior_structure/',
            ...
            num_base_shots=None)),
    val=dict(
        type='FewShotdcDataset',
        ann_cfg=[
            dict(
                type='ann_file', ann_file='data/DC/dior_structure/ImageSets/Main/val.txt')
        ],
        img_prefix='data/DC/dior_structure/',
        pipeline=[
            dict(type='LoadImageFromFile'),
    ...
    test=dict(
        ...
        ann_cfg=[
            dict(
                type='ann_file',
                ann_file=
                'data/DC/dior_structure/ImageSets/Main/text_normal.txt'
            )
        ],
        img_prefix='data/DC/dior_structure/',
        pipeline=[
...
log_level = 'INFO'
load_from = './work_dirs_dc/meta-rcnn_r101_c4_8xb4_voc-split5_base-training/latest.pth'
resume_from = None
workflow = [('train', 1)]
use_infinite_sampler = True
seed = 42
work_dir = './work_dirs_dc/meta-rcnn_r101_c4_8xb4_voc-split5_20shot-fine-tuning'
gpu_ids = [0]
