...
dc_SPLIT = dict(
    ALL_CLASSES_SPLIT1=('R2', 'r1'),
    NOVEL_CLASSES_SPLIT1=('r1',),
    BASE_CLASSES_SPLIT1=('R2',),
    ALL_CLASSES_SPLIT2=('R2', 'r1'),
    NOVEL_CLASSES_SPLIT2=('r1',),
    BASE_CLASSES_SPLIT2=('R2',),
    ALL_CLASSES_SPLIT3=('R2', 'r1'),
    NOVEL_CLASSES_SPLIT3=('r1',),
    BASE_CLASSES_SPLIT3=('R2',),
    ALL_CLASSES_SPLIT4=('R2', 'r1'),
    NOVEL_CLASSES_SPLIT4=('r1',),
    BASE_CLASSES_SPLIT4=('R2',),
    ALL_CLASSES_SPLIT5=('R2', 'r1'),
    NOVEL_CLASSES_SPLIT5=('r1',),
    BASE_CLASSES_SPLIT5=('R2',), )



@DATASETS.register_module()
class FewShotdcDatasetSkipNovel(BaseFewShotDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        for img_id in img_ids:
            filename = osp.join('JPEGImages', f'{img_id}.png')
...
@DATASETS.register_module()
class FewShotdcDatasetV2(FewShotdcDatasetSkipNovel):
...
@DATASETS.register_module()
class FewShotdcDataset(FewShotdcDatasetV2):
...
@DATASETS.register_module()
class FewShotdcDefaultDataset(FewShotdcDataset):
    """Dataset with some pre-defined dc annotation paths.

    :obj:`FewShotdcDefaultDataset` provides pre-defined annotation files
    ...
    different from :obj:`FewShotdcDataset`. The `ann_cfg' should contain
    ...
    dc_benchmark = {
        f'{cls}_SPLIT{split}_{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'data/DC/few_shot_ann/dior/benchmark_{shot}shot/'
                         f'box_{shot}shot_{class_name}_train.txt',
                ann_classes=[class_name])
            for class_name in dc_SPLIT[f'{cls}_SPLIT{split}']
        ]
    ...
    def load_annotations(self, ann_folder):
        """
            Args:
                ann_folder: folder that contains dc v1 annotations xml files
        """
        self.cat2label = {cat: i for i, cat in enumerate(self.CLASSES)}
        img_ids = []
        ...
        benchmarks = self.dc_benchmark[classes + '_' + split + '_' + setting]
...
@DATASETS.register_module()
class FewShotdcDefaultDatasetWithSeedK(FewShotdcDataset):
    ...
    dc_benchmark = {
        f'{cls}_SPLIT{split}_{shot}SHOT_SEED{seed}': [
            dict(
                type='ann_file',
                ann_file=f'data/few_shot_ann/dc_cir_fsd/seed{seed}/'
                         f'box_{shot}shot_{class_name}_train.txt',
                ann_classes=[class_name])
            for class_name in dc_SPLIT[f'{cls}_SPLIT{split}']
        ]
    ...
    def load_annotations(self, ann_folder):
        ...
        seed = self.seed
        classes = self.classes.split('_SPLIT')[0]
        benchmarks = self.dc_benchmark[classes + '_' + split + '_' +
...
@DATASETS.register_module()
class FewShotdcCopyDataset(FewShotdcDataset):
    """Copy other dc few shot datasets' `data_infos` directly.
    ...
    def ann_cfg_parser(self, ann_cfg: Union[List[Dict], Dict]) -> List[Dict]:
        ...
        if isinstance(ann_cfg, dict):
            assert ann_cfg.get('data_infos', None) is not None, \
                ...
                f'FewShotdcCopyDataset require data_infos.'
