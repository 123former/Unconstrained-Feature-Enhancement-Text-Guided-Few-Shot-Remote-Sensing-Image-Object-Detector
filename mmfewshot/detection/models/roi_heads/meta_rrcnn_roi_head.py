...
class MetaRRCNNRoIHead(StandardRoIHead):
    ...
    def _bbox_forward_train(self, query_feats: List[Tensor],
        ...
        bbox_results = None
        all_bbox_results = dict(cls_score=[], bbox_pred=[])
        for img_id in range(batch_size):
            ...
            for i in range(support_feat.size(0)):
                ...
                if support_gt_labels[i] == random_query_label:
                    if self.aug_feat is not None:
                        bbox_results = self._bbox_forward(
                            query_roi_feats[start:end],
                            support_feat[i].unsqueeze(0),
                            self.aug_feat[start:end])
                    else:
                        bbox_results = self._bbox_forward(
                            query_roi_feats[start:end],
                            support_feat[i].unsqueeze(0))
                    all_bbox_results['cls_score'].append(bbox_results['cls_score'])
                    all_bbox_results['bbox_pred'].append(bbox_results['bbox_pred'])
        ...
        all_bbox_results.update(loss_bbox=loss_bbox)
        all_bbox_results['cls_score'] = torch.cat(all_bbox_results['cls_score'], 0)
        all_bbox_results['bbox_pred'] = torch.cat(all_bbox_results['bbox_pred'], 0)
        return all_bbox_results
    ...
    def _bbox_forward(self, query_roi_feats: Tensor,
        ...
        if aug_feat is not None:
            roi_feats = torch.cat([roi_feats, aug_feat], 1)
    ...
    def simple_test_bboxes(
        ...
        for class_id in support_feats_dict.keys():
            ...
            if self.aug_feat is not None:
                bbox_results = self._bbox_forward(query_roi_feats, support_feat, self.aug_feat)
            else:
