...
class DecoupleBBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(DecoupleBBoxHead, self).__init__(init_cfg)
        ...
        if self.with_cls:
            ...
            self.fc_cls = build_linear_layer(
                ...
                out_features=cls_channels - novel_class)

            self.fc_novel_cls = build_linear_layer(
                self.cls_predictor_cfg,
                in_features=in_channels,
                out_features=novel_class + 1)
        if self.with_reg:
            out_dim_reg = 4 if reg_class_agnostic else 4 * (num_classes - novel_class)
            out_novel_dim_reg = 4 if reg_class_agnostic else 4 * novel_class
            self.fc_reg = build_linear_layer(
                self.reg_predictor_cfg,
                in_features=in_channels,
                out_features=out_dim_reg)

            self.fc_novel_reg = build_linear_layer(
                self.reg_predictor_cfg,
                in_features=in_channels,
                out_features=out_novel_dim_reg)
        ...
        if with_meta_cls_loss:
            self.fc_meta = nn.Linear(meta_cls_in_channels, num_classes - novel_class)
            self.fc_novel_meta = nn.Linear(meta_cls_in_channels, novel_class)
            self.meta_cls_loss_weight = meta_cls_loss_weight
            self.loss_meta_cls = build_loss(copy.deepcopy(loss_meta))
        self.affine = AffineLayer(num_channels=in_channels, bias=True)
        self.affine_novel = AffineLayer(num_channels=in_channels, bias=True)
    ...
    def forward(self, x):
        ...
        # x = self.affine(decouple_layer(x, 1))
        cls_score = self.fc_cls(x) if self.with_cls else None
        bbox_pred = self.fc_reg(x) if self.with_reg else None

        # x_novel = self.affine_novel(decouple_layer(x, 1))
        cls_novel_score = self.fc_novel_cls(x) if self.with_cls else None
        bbox_novel_pred = self.fc_novel_reg(x) if self.with_reg else None

        out_cls_score = torch.cat((cls_score[:, :-1], cls_novel_score[:, :-1]), dim=-1)
        out_cls_score = torch.cat((out_cls_score, ((cls_score[:, -1] + cls_novel_score[:, -1]) / 2).unsqueeze(1)),
                                  dim=-1)
        out_bbox_pred = torch.cat((bbox_pred, bbox_novel_pred), dim=-1)
        return out_cls_score, out_bbox_pred
    ...
    def forward_meta_cls(self, support_feat):
        ...
        meta_novel_cls_score = self.fc_novel_meta(support_feat)
        out_cls_score = torch.cat((meta_cls_score, meta_novel_cls_score), dim=-1)
        return out_cls_score
