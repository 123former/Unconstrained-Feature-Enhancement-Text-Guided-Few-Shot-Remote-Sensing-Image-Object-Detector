...
class Shared2FCRBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        self.aug_feat_cls = None
        self.aug_feat_reg = None
        ...
        if self.with_cls:
            ...
            self.fc_cls = build_linear_layer(
                ...
                in_features=self.cls_last_dim + add,
    ...
    def forward(self, x):
        ...
        if self.aug_feat_cls is not None:
            x_cls = torch.cat([x_cls, self.aug_feat_cls], 1)
...
class CosineSimBBoxHead(ConvFCBBoxHead):
        ...
        if self.add_channel:
            if self.with_cls:
                self.fc_cls = nn.Linear(
                    self.cls_last_dim + 256, self.num_classes + 1, bias=False)

            # if self.with_reg:
            #     out_dim_reg = 4 * self.num_classes
            #     self.fc_reg = nn.Linear(self.cls_last_dim + 256, out_dim_reg)
        else:
            if self.with_cls:
                self.fc_cls = nn.Linear(
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if self.aug_feat_reg is not None:
            x_reg = torch.cat([x_reg, self.aug_feat_reg], 1)
