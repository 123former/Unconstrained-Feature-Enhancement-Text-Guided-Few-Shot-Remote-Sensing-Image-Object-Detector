...
class BBoxHeadAsy(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(BBoxHeadAsy, self).__init__(init_cfg)
    ...
    @auto_fp16()
    def forward(self, x, decouple=False):
        if decouple:
            x_cls = x[0]
            x_reg = x[1]
            if self.with_avg_pool:
                if x_cls.numel() > 0:
                    ...
                    # avg_pool does not support empty tensor,
                    # so use torch.mean instead it
                    x_cls = torch.mean(x_cls, dim=(-1, -2))
            ...
            if self.with_avg_pool:
                if x_reg.numel() > 0:
                    ...
                    # avg_pool does not support empty tensor,
                    # so use torch.mean instead it
                    x_reg = torch.mean(x_reg, dim=(-1, -2))
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                if self.custom_activation:
                    ...
                    losses['acc'] = (accuracy(cls_score, labels[:, 0]) + accuracy(cls_score, labels[:, 1])) / 2
        if bbox_pred is not None:
            bg_class_ind = self.num_classes - 7
            # 0~self.num_classes-1 are FG, self.num_classes is BG
            pos_inds = (labels[:, 0] >= 0) & (labels[:, 0] < bg_class_ind)
            ...
            if pos_inds.any():
                ...
                if self.reg_class_agnostic:
                    ...
                    pos_bbox_pred = bbox_pred.view(
                        bbox_pred.size(0), -1,
                        4)[pos_inds.type(torch.bool),
                           labels[:, 0][pos_inds.type(torch.bool)]]
                losses['loss_bbox'] = self.loss_bbox(
                    pos_bbox_pred,
                    bbox_targets[pos_inds.type(torch.bool)],
                    bbox_weights[pos_inds.type(torch.bool)],
