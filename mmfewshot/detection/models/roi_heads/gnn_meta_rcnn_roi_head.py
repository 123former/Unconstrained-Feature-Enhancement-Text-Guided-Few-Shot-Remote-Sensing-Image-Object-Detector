...
class GNNMetaRCNNRoIHead(StandardRoIHead):
    ...
    def _bbox_forward_train(self, query_feats: List[Tensor],
        ...
        for img_id in range(batch_size):
            ...
            support_feat_, query_roi_feats_ = self.gnn._process_per_class(query_rois[start:end], query_roi_feats[start:end], support_feat)
            ...
            for i in range(support_feat_.size(0)):
                ...
                if support_gt_labels[i] == random_query_label:
                    bbox_results = self._bbox_forward(query_roi_feats_, support_feat_[i].unsqueeze(0))
        ...
        if self.bbox_head.with_meta_cls_loss:
            # support_gt_labels.append(torch.tensor([len(support_feat)-1]).cuda())
