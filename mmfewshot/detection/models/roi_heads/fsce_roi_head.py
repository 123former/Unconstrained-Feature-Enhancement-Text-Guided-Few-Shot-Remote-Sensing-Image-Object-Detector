...
class FscedRoIHead(StandardRoIHead):
    def __init__(self,
    ...
    def _bbox_forward(self, x, rois):
        ...
        multi_feat = self.bbox_head.forward_muli_conv(bbox_feats)

        # for feat in multi_feat:
        #     roi_feats_list.append(feat.mean(3).mean(2))
            
        cls_score, bbox_pred = self.bbox_head(multi_feat[0])
        
        roi_feats_list=[]
        for feat in multi_feat:
            roi_feats_list.append(feat.mean(3).mean(2))
        reason_cls_score = self.bbox_head.forward_reason_cls(roi_feats_list)

        bbox_results = dict(
            cls_score=cls_score, bbox_pred=bbox_pred, bbox_feats=bbox_feats, reason_cls_score=reason_cls_score)
        return bbox_results
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        loss_bbox = self.bbox_head.loss(bbox_results['cls_score'],
                                        ...
                                        *bbox_targets)
        reason_cls_score=bbox_results['reason_cls_score']
        case_dict = self.conver_cls(bbox_targets[0])
        
        loss_reason_cls = self.bbox_head.loss_reason(reason_cls_score, 
                                                     case_dict,
                                                    torch.ones_like(bbox_targets[1]),
                                                    flag='query')
        
        for key in loss_reason_cls.keys():
            loss_bbox[key]=loss_reason_cls[key]
