...
class AugMetaRCNNRoIHead(StandardRoIHead):
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            cal_loss = self.cal_loss()
            losses.update(cal_loss=cal_loss)
    ...
    def _bbox_forward_train(self, query_feats: List[Tensor],
        ...
        support_feat, query_roi_feats = self.shared_head.aug_feat(support_feat, query_roi_feats)
        ...
        for img_id in range(batch_size):
            ...
            for i in range(support_feat.size(0) - 1):
        ...
        if self.bbox_head.with_meta_cls_loss:
            ...
            class_num = len(support_gt_labels)
            support_gt_labels.append(torch.tensor([class_num]).cuda())
    ...
    def cal_loss(self):
        ...
        fore_num = weight_c.shape[0] - 1
        avg_for_cos_sim = 0

        cos_sim = torch.cosine_similarity(weight_p, weight_c, 1)
    ...
    def dict2tensor(self, support_feats_dict):
        feats_list = []
        keys_list = []
        for cls in support_feats_dict.keys():
            feat = support_feats_dict[cls]
            keys_list.append(cls)
            feats_list.append(feat)

        feats_tensor = torch.cat(feats_list, 0).cuda()
        return feats_tensor, keys_list

    def tensor2dict(self, feats_tensor, keys_list):
        support_feats_dict = dict()
        for i, key in enumerate(keys_list):
            support_feats_dict[key] = feats_tensor[i, :]

        return support_feats_dict
    ...
    def simple_test_bboxes(
        ...
        support_feat, support_keys_list = self.dict2tensor(support_feats_dict)
        query_roi_feats = self.extract_query_roi_feat(query_feats, rois)
        support_feat, query_roi_feats = self.shared_head.aug_feat(support_feat, query_roi_feats)
        support_feats_dict = self.tensor2dict(support_feat, support_keys_list)
