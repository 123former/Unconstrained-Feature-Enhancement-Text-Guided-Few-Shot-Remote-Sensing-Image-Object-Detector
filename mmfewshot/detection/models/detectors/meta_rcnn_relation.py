...
class MetaRelationRCNN(QuerySupportDetector):
    ...
    def forward_train(self,
        ...
        roi_losses = self.roi_head.forward_train(
            ...
            support_gt_bboxes=support_data['gt_bboxes'],
            support_gt_labels=support_data['gt_labels'],
            support_reason_cls=support_data['reason_cls'],
    ...
    def forward_model_init(self,
        ...
        _, roi_feat = self.roi_head.extract_support_feats(feats)

        self._forward_saved_support_dict['gt_labels'].extend(gt_labels)
        self._forward_saved_support_dict['roi_feats'].extend(torch.unsqueeze(roi_feat[-1], 0))
        return {'gt_labels': gt_labels, 'roi_feat': roi_feat[-1]}
    ...
    def model_init(self):
        ...
        for class_id in class_ids:
            feats_list.append(roi_feats[gt_labels == class_id].mean([0], True))
        self.inference_support_dict=torch.cat(feats_list, 0)
        # set the init flag
