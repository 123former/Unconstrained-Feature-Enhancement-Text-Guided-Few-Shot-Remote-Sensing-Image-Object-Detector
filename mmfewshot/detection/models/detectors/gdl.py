...
class AffineLayer(nn.Module):
    def __init__(self, num_channels, bias=False, reduce_channels=False):
        super(AffineLayer, self).__init__()
        if reduce_channels:
            weight = torch.FloatTensor(1, num_channels).fill_(1)
            self.weight = nn.Parameter(weight, requires_grad=True)

            self.bias = None
            if bias:
                bias = torch.FloatTensor(1, num_channels).fill_(0)
