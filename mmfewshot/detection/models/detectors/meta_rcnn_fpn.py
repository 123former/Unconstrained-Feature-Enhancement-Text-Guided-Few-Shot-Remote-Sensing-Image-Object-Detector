...
class FPNMetaRCNN(QuerySupportDetector):
    ...
    def __init__(self,
        ...
        # self.init_fuse_layer()
    
    def init_fuse_layer(self):
    ...
    def fuse_feats(self, feats):
        (a, b)=feats[-2].shape[-2], feats[-2].shape[-1]
        adaverage_pool = nn.AdaptiveAvgPool2d(output_size=(a, b)) # 输出大小的尺寸指定为100*100
        pool_out1 = adaverage_pool(feats[0])
        pool_out2 = adaverage_pool(feats[1])
        inter_out1 = F.interpolate(feats[-1], scale_factor=2)
        pool_out3 = adaverage_pool(inter_out1)
     
        out=[torch.cat([pool_out1, pool_out2, pool_out3, feats[-2]], 1)]
        # pdb.set_trace()
        # out=[self.fusion_layer(pool_out1+pool_out2+pool_out3+feats[-2])]
        return out
    ...
    def forward_train(self,
        ...
        query_feats = self.fuse_feats(query_feats)
        support_feats = self.fuse_feats(support_feats)
