...
class FewShotDIORDataset(BaseFewShotDataset):
    ...
    def load_annotations_xml(
        ...
        for img_name in img_names:
            ...
            xml_path = osp.join(self.img_prefix, Annotation_dir,
                                f'{img_id}.xml')
            ...
            ann_info = self._get_xml_ann_info(img_id, classes, Annotation_dir)
            data_infos.append(
                dict(
                    id=img_id,
                    filename=filename,
...
class FewShotDIORDefaultDataset(FewShotDIORDataset):
    ...
    dior_benchmark = {
        f'SPLIT{split}_{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'/home/f523/disk1/sxp/mmfewshot/data/few_shot_ann/dior/benchmark_{shot}shot/'
