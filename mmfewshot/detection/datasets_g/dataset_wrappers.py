...
from .multi_case import reason_cls
import pdb
import torch
...
class NWayKShotDataset:
    ...
    def get_multi_case(self, data_dict):
        labels_list = data_dict['gt_labels']
        data_dict['reason_cls'] = dict()
        data_dict['reason_cls']['gt_supclass_cls'] = []
        data_dict['reason_cls']['gt_man_made_cls'] = []
        data_dict['reason_cls']['gt_envir_cls'] = []
        for label in labels_list:
            label_item = label.items()
            for i, cls in self.query_dataset.CLASSES:
                if label_item == i:
                    cls = cls.split(' ', '_')
                    reason_id = reason_cls[cls]
                    data_dict['reason_cls']['gt_supclass_cls'].append(torch.tensor([reason_id[0]]))
                    data_dict['reason_cls']['gt_man_made_cls'].append(torch.tensor([reason_id[1]]))
                    data_dict['reason_cls']['gt_envir_cls'].append(torch.tensor([reason_id[2]]))
        return data_dict
    ...
    def __getitem__(self, idx: int) -> Union[Dict, List[Dict]]:
        if self.multi_case:
            if self._mode == 'query':
                ...
                return self.get_multi_case(self.query_dataset.prepare_train_img(idx, 'query'))
            elif self._mode == 'support':
                # loads one batch of data in support pipeline
                b_idx = self.batch_indices[idx]
                batch_data = [
                    self.get_multi_case(self.support_dataset.prepare_train_img(idx, 'support',
                                                                               [gt_idx]))
