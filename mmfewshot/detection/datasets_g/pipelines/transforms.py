    ...
    def get_multi_case(self, data_dict):
        ...
        for label in labels_list:
            ...
            if label == len(self.classes):
                data_dict['reason_cls']['gt_supclass_cls'].append(torch.tensor([113]))
                data_dict['reason_cls']['gt_man_made_cls'].append(torch.tensor([202]))
                data_dict['reason_cls']['gt_envir_cls'].append(torch.tensor([303]))
        return data_dict

    def __call__(self, results) -> Dict:

        results_ = copy.deepcopy(results)
        self.get_multi_case(results_)
        # print(results_.keys())
        return results_
